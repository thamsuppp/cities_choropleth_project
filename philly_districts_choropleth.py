...
def prepare_gdf(district_list, variable_name, level_name):
    ...
    #Multi variable
        ...
        gdf = gdf.merge(multi_var_level_df, on = 'zipcode', how = 'outer')

    #Drop rows where polygon is none
    gdf = gdf.dropna()
...
### APP LAYOUT ###
app.layout = html.Div([
    html.Div([html.H1("Philadelphia Districts")],
...
def update_figure(layer_opacity_slider_value, multi_variable_levels_dropdown_value, layer_checkbox_value, 
transform_dropdown_value, variable_dropdown_value):
    ...
    if transform_dropdown_value == '% of Population':
        population_gdf = prepare_gdf(philly, 'population', None)
        series_to_divide_by = population_gdf['population'] / 100
    elif transform_dropdown_value == 'Density (per square mile)':
        population_gdf = prepare_gdf(philly, 'land_area_in_sqmi', None)
        series_to_divide_by = population_gdf['land_area_in_sqmi']
    else:
        series_to_divide_by = 1

    #Create GDF with desired variable
    gdf = prepare_gdf(philly, variable_dropdown_value, multi_variable_levels_dropdown_value)

    #Transform the variable
    if transform_dropdown_value != 'Value':
        gdf[variable_dropdown_value] = gdf[variable_dropdown_value] / series_to_divide_by
